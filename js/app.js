...
/* ---------- Supabase config (safe publishable key only) ---------- */
...
function defaults(){
  return {
    ...
    myRecipes: [],
    myEquipment: [],
    overrides: {},        // id -> {name, image} for built-in recipes
    hidden: [],           // hidden built-in recipe ids
    pantry: {},           // recipeId -> { ingredientName: true }
...
function calc(r){
  ...
  const ferm = (r.ferm||[]).map(f=>({ name:f[0], kg:f[1]*sf, price:f[2], owned:pantried(r.id,f[0]) }));
  const totalGrain = ferm.reduce((s,f)=>s+f.kg,0);
  const preboil = STATE.batchSize + eq.kettleLoss + eq.boilOff; // ~1hr boil
  const og = r.og; // use stored target OG as the working gravity (stable, matches recipe intent)
  const hops = (r.hops||[]).map(h=>{
    const g = h[1]*sf;
    const util = h[3]==="Boil"?1 : h[3]==="Whirlpool"?0.35 : 0;
    const ibu = h[3]==="Dry Hop" ? 0 : (g*(h[2]/100)*1000*tinseth(h[4],og)*util)/preboil;
    return { name:h[0], g, aa:h[2], stage:h[3], time:h[4], ibu, owned:pantried(r.id,h[0]), price:HOP_PRICE.default };
  });
...
function dash(){
  ...
  $("#app").innerHTML = `
    ...
    <div class="card stats">
      ${stat("Est. OG", fmt(r.og,3))}${stat("Est. FG", fmt(r.fg,3))}${stat("ABV", r.abv+"%")}
...
function library(){
  ...
  $("#app").innerHTML = `
    ...
    <div class="card list" style="padding:0;overflow:hidden;">
      <table><thead><tr><th style="width:34px;"></th><th style="width:44px;"></th><th>Recipe</th><th>Style</th><th>ABV</th><th>IBU</th><th>Batch</th><th>Source</th><th>Updated</th><th style="width:130px;"></th></tr></thead><tbody id="rows"></tbody></table>
    </div>`;
  const rows = ()=>{
...
function findbrew(){
  $("#app").innerHTML = `
    <div class="card"><h3>Find a Brew</h3>
      <p class="desc">A static site can't safely call a live AI search API (that needs a server-side key). Instead, ask an AI assistant in chat for a recipe in the JSON format below, paste it here, and it loads straight into Create a Brew. Or use Quick Generate to adapt a style template.</p>
    </div>
    <h2 class="sec">Paste a recipe</h2>
    <div class="card">
      <textarea id="paste" style="width:100%;height:170px;font-family:monospace;" placeholder='Paste JSON, e.g. {"name":"Example Pale","style":"APA","baseBatch":40,"abv":5.4,"ibu":32,"ferm":[["Pale Ale Malt",8.5,4.85]],"hops":[["Citra",40,12,"Whirlpool",20]]}'></textarea>
  ...
  $("#parse").onclick=()=>{
    ...
    let d; try{ const o=JSON.parse(t); if(o.ferm&&o.hops){d=o;} }catch{}
    if(!d){$("#pres").innerHTML=`<span class="warn">Could not read that as a JSON recipe. Check the format.</span>`;return;}
    d.id=null; d.custom=true; d.baseBatch=d.baseBatch||STATE.batchSize; d.water=d.water||Object.assign({},FLORAVILLE_WATER);
    d.og=d.og||1.05; d.fg=d.fg||1.01; d.abv=d.abv||5; d.ibu=d.ibu||30; d.yeast=d.yeast||"US-05"; d.yeastForm=d.yeastForm||"Dry"; d.atten=d.atten||0.78;
...
function hopsTab(){
  ...
  $("#app").innerHTML = `
    ...
    <div class="card stats">${stat("Total IBU",fmt(c.ibu,1))}${stat("Target IBU",r.ibu)}${stat("Total hops",fmt(c.totalHops,0)+" g")}</div>`;
  bindOwn(r);
}
...
function water(){
  $("#app").innerHTML = `
    ...
    <div class="card"><table><thead><tr><th>Ion</th><th>Source</th><th>Style target</th><th>Difference</th></tr></thead><tbody>
      ${WATER_IONS.map(i=>{const t=selected().water[i],s=STATE.sourceWater[i],d=t-s;return `<tr><td>${WATER_LABELS[i]}</td><td>${fmt(s,1)}</td><td>${fmt(t,1)}</td><td style="color:${Math.abs(d)>40?'#b42318':'#19753c'}">${d>0?'+':''}${fmt(d,1)}</td></tr>`;}).join("")}
...
function fermentation(){
  ...
  $("#app").innerHTML = `
    <div class="card"><h3>Yeast and schedule</h3><table>
      ${row("Yeast",esc(r.yeast))}${row("Form",r.yeastForm)}${row("Attenuation",Math.round(r.atten*100)+"%")}${row("Pitch temp",r.tempLo+"°C")}${row("Upper temp",r.tempHi+"°C")}${row("Expected FG",fmt(r.fg,3))}
    </table></div>
...
function readme(){
  $("#app").innerHTML = `
    ...
    <div class="card"><h3>Custom logo and recipe photos</h3>
      <p>The BrewGenge crest and every recipe icon are built-in vector art, so nothing is ever a broken image. To use real photos: add <code>img/logo.png</code> to your repo for the sidebar logo, or just upload a photo to any brew with the 🖼 button.</p>
    </div>
    <div class="card"><h3>Hosting on GitHub Pages</h3><ol>
      <li>Upload <code>index.html</code>, <code>css</code>, <code>js</code> and <code>supabase</code> to the repo root.</li>
...
function importJSON(e){
  ...
  rd.onload=()=>{
    try{
      ...
      let list = o.format==="brewgenge-recipe-pack" ? o.recipes
               : o.format==="brewgenge-recipe" ? [o.recipe]
               : Array.isArray(o) ? o : [o];
      let n=0;
      list.forEach(r=>{ const c=JSON.parse(JSON.stringify(r)); c.id=null; c.custom=true; saveRecipe(c); n++; });
      alert(`Imported ${n} recipe${n===1?'':'s'}${list.some(r=>r.image)?', including embedded images':''}.`);
      render();
    }catch(err){ alert("That file is not a valid BrewGenge recipe or pack."); }
  };
  rd.readAsText(f);
  e.target.value="";
}
...
/* ============================================================
   Boot
   ============================================================ */
$("#crest").innerHTML = BREWGENGE_LOGO_SVG;
